...
<div class="container">
    ...
    <?php 
          ...
          if (!$conn) {
            ...
            $result = $conn->query("SELECT COUNT(imie) as klienci FROM `klienci`;");
            $row = $result->fetch_assoc();
            echo "Liczba klientów: " . $row['klienci'];
    ...
    <form action="" method="post" name="zamowienie">Sprawdz swoje zamowienie: <br>
        <label for="nrzamowienia">Podaj numer zamówienia
   ...
   if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['nrzamowienia'])) {
       $id_zamowienia = $_POST['nrzamowienia'];
   
       if (!empty($id_zamowienia)) {
           $zap = "SELECT produkty.nazwa, produkty.cena, szczegółyzamówienia.ilość,
           (produkty.cena * szczegółyzamówienia.ilość) AS wartosc_calkowita 
           FROM szczegółyzamówienia
           JOIN produkty ON szczegółyzamówienia.id_produktu = produkty.id_produktu 
           WHERE szczegółyzamówienia.id_zamówienia = ?";
   
           $stmt = $conn->prepare($zap);
           ...
           if ($result->num_rows > 0) {
               ...
               while ($row = $result->fetch_assoc()) {
                   echo "Produkt: " . htmlspecialchars($row['nazwa']) . "<br>";
                   echo "Cena: " . htmlspecialchars($row['cena']) . " zł<br>";
                   echo "Ilość: " . htmlspecialchars($row['ilość']) . "<br>";
                   echo "Wartość całkowita: " . htmlspecialchars($row['wartosc_calkowita']) . " zł<br><br>";
               }
           } else {
               echo "Brak zamówienia o podanym numerze.";
           }
        }
           $stmt->close();
       } 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            echo "<p>Twoje zamówienie nr: ". $zamowienie_id . "Zamówienie zostało złożone. Dziękujemy!</p>";
        } else {
